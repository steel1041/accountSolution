...
namespace Account
{
    public class Account : SmartContract
    {
        ...
        * addr,name,type,operated*/
        ...
        private const string FEE_ACCOUNT = "fee_account";
        ...
        private const string APPROVE_HZ = "3";

        private const string FEE_CONFIG = "fee_config";
        ...
        public static object Main(string method, object[] args)
        {
            ...
            else if (Runtime.Trigger == TriggerType.Application)
            {
                ...
                if (method == "approveOneKey") return ApproveOneKey((byte[])args[0], (string)args[1], (string)args[2], (string)args[3], (string)args[4],(BigInteger)args[5], (BigInteger)args[6], (BigInteger)args[7]);
                ...
                if (method == "transfer") return Transfer((byte[])args[0], (string)args[1], (string)args[2], (string)args[3], (string)args[4], (BigInteger)args[5]);
                ...
                if (method == "setConfig") return SetConfig((string)args[0],(BigInteger)args[1]);

                if (method == "getConfig") return GetConfig((string)args[0]);
        ...
        /// <param name="type">1/total or 2/single 3/hz</param>
        ...
        public static bool ApproveTransfer(byte[] addr, string srcName,string destName, string type, string assetType, string asset, BigInteger mount)
        {
            ...
            if (asset.Length <= 0)
                throw new InvalidOperationException("The parameter asset SHOULD be longer than 0.");
            ...
            //hz
            else if (type == APPROVE_HZ) {
                StorageMap authHz = Storage.CurrentContext.CreateMap(nameof(authHz));
                authHz.Put(key, mount);
            }
        ...
        /// <param name="assetType">nep5 or nep55</param>
        /// <param name="asset">nep5:sds_account/nep55:SD-HELLO</param>
        /// <param name="totalMount">approve totalMount</param>
        /// <param name="singleMount">approve singleMount</param>
        /// <param name="hzMount">approve hzMount</param>
        /// <returns>boolean</returns>
        [DisplayName("approveOneKey")]
        public static bool ApproveOneKey(byte[] addr, string srcName, string destName,string assetType, string asset, BigInteger totalMount,BigInteger singleMount,BigInteger hzMount)
        {
            if (!Runtime.CheckWitness(addr)) return false;
            ...
            if (totalMount < 0 || singleMount < 0 || hzMount<0)
            ...
            byte[] keyTotal = addr.Concat(destAddr).Concat(APPROVE_TOTAL.AsByteArray()).Concat(assetType.AsByteArray()).Concat(asset.AsByteArray());
            byte[] keySingle = addr.Concat(destAddr).Concat(APPROVE_SINGLE.AsByteArray()).Concat(assetType.AsByteArray()).Concat(asset.AsByteArray());
            byte[] keyHz = addr.Concat(destAddr).Concat(APPROVE_HZ.AsByteArray()).Concat(assetType.AsByteArray()).Concat(asset.AsByteArray());

            //total
            StorageMap authTotal = Storage.CurrentContext.CreateMap(nameof(authTotal));
            authTotal.Put(keyTotal, totalMount);
            Approved(addr, destAddr, srcName.AsByteArray(), destName.AsByteArray(), APPROVE_TOTAL.AsByteArray(), assetType.AsByteArray(), asset.AsByteArray(), totalMount);

            //single
            StorageMap authSingle = Storage.CurrentContext.CreateMap(nameof(authSingle));
            authSingle.Put(keySingle, singleMount);
            Approved(addr, destAddr, srcName.AsByteArray(), destName.AsByteArray(), APPROVE_SINGLE.AsByteArray(), assetType.AsByteArray(), asset.AsByteArray(), singleMount);

            //hz
            StorageMap authHz = Storage.CurrentContext.CreateMap(nameof(authHz));
            authHz.Put(keyHz, hzMount);
            Approved(addr, destAddr, srcName.AsByteArray(), destName.AsByteArray(), APPROVE_HZ.AsByteArray(), assetType.AsByteArray(), asset.AsByteArray(), hzMount);
            return true;
        }
        ...
        public static BigInteger GetApproveInfo(string srcName, string destName, string type, string assetType, string asset)
        {
            ...
            {
                StorageMap authHz = Storage.CurrentContext.CreateMap(nameof(authHz));
                ret = authHz.Get(key).AsBigInteger();
            }
            return ret;
        }
        ...
        public static bool UserTransfer(byte[] addr, string srcName, string destName,string assetType, string assetName, BigInteger mount)
        {
            ...
            byte[] keyHz = srcAddr.Concat(destAddr).Concat(APPROVE_HZ.AsByteArray()).Concat(assetType.AsByteArray()).Concat(assetName.AsByteArray());
            ...
            //频率设置
            BigInteger ret = authHz.Get(keyHz).AsBigInteger();
            var currentHeight = Blockchain.GetHeight();

            //上次转账的高度
            StorageMap hzStore = Storage.CurrentContext.CreateMap(nameof(hzStore));
            BigInteger lastHeight = hzStore.Get(keyHz).AsBigInteger();
            if((currentHeight - lastHeight)<ret)
                throw new InvalidOperationException("The transfer operation is too hz.");
            ...
            if (assetType == ASSET_NEP5)
            {
                ...
                StorageMap asset = Storage.CurrentContext.CreateMap(nameof(asset));
                byte[] fee_account = asset.Get(FEE_ACCOUNT);
                if (fee_account.Length != 20) {
                    fee_account = committee;
                }
                byte[] keyFee = concatKey(concatKey(fee_account, ASSET_NEP5), assetName);
                BigInteger feeLocked = nep5AssetStore.Get(keyFee).AsBigInteger();

                //手续费比例,0-100
                StorageMap config = Storage.CurrentContext.CreateMap(nameof(config));
                BigInteger fee = config.Get(FEE_CONFIG).AsBigInteger();
                BigInteger feeMount = 0;
                if (fee >= 0 && fee <= 100) {
                    feeMount = mount * fee / 100;
                }

                nep5AssetStore.Put(key, nep5Locked - mount);
                if (destAddr != fee_account)
                {
                    nep5AssetStore.Put(keyDest, destLocked + mount - feeMount);
                    nep5AssetStore.Put(keyFee, feeLocked + feeMount);
                }
                else
                {
                    nep5AssetStore.Put(keyDest, destLocked + mount - feeMount);
                ...
                //记录转账数据
                AccoutOperated(destAddr, assetName.AsByteArray(), (int)ConfigTranType.TRANSACTION_TYPE_TRANSFER, mount - feeMount);
                if (feeMount > 0)
                {
                    //记录手续费转账
                    AccoutOperated(fee_account, assetName.AsByteArray(), (int)ConfigTranType.TRANSACTION_TYPE_TRANSFER, feeMount);
                }
            }
            else if (assetType == ASSET_NEP55)
            {
                ...
                byte[] keyFee = concatKey(concatKey(fee_account, ASSET_NEP55), assetName);
                BigInteger feeLocked = nep55AssetStore.Get(keyFee).AsBigInteger();
                ...
                if (destAddr != fee_account)
                {
                    nep55AssetStore.Put(keyDest, destLocked + mount - feeMount);
                    nep55AssetStore.Put(keyFee, feeLocked + feeMount);
                }
                else
                {
            ...
            hzStore.Put(keyHz, currentHeight);
            return true;
        }
        ...
        public static bool Transfer(byte[] addr, string srcName, string destName, string assetType, string assetName, BigInteger mount)
        {
            if (!Runtime.CheckWitness(addr)) return false;
        ...
        public static bool SetConfig(string key, BigInteger value)
        {
            ...
            if (value<0)
                throw new InvalidOperationException("The parameters value SHOULD be larger than 0.");
